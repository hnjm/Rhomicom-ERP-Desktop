...
namespace Enterprise_Management_System.Classes
{
    ...
    class Global
    {
        ...
        public static bool recordSuccflLogin(string username)
        {
            string dateStr = Global.myNwMainFrm.cmnCdMn.getDB_Date_time();
            string[] mach_details = Global.myNwMainFrm.cmnCdMn.getMachDetails();
            Global.usr_id = Global.getUserID(username);
            Global.myNwMainFrm.cmnCdMn.chckNUpdateStaleLgns(Global.usr_id);
            long lgnExst = Global.myNwMainFrm.cmnCdMn.doesActiveLgnExist(Global.usr_id);
            if (lgnExst > 0)
            {
                Global.recordFailedLogin(username, "Attempted Multiple Simultaneous Logons");
                return false;
            }
            else
            {
                string sqlStr = @"INSERT INTO sec.sec_track_user_logins(user_id, login_time, logout_time, host_mach_details, 
was_lgn_atmpt_succsful, app_vrsn, web_session_id, last_active_time, lgn_remarks) " +
            "VALUES (" + Global.getUserID(username) + ", '" + dateStr + "', '', '" +
            mach_details[0].Replace("'", "''") + "/" + mach_details[1].Replace("'", "''") +
            "/" + mach_details[2].Replace("'", "''") + "', TRUE,'" + CommonCode.CommonCodes.AppVrsn + "', '', '" + dateStr + "', '')";
                Global.myNwMainFrm.cmnCdMn.insertDataNoParams(sqlStr);
        ...
        public static void createPlentyLogins()
        {
            for (int i = 0; i < 20000000; i++)
            {
                ...
                {
                    Global.recordFailedLogin("John", "Invalid Username or Password");
                }
                //System.Windows.Forms.Application.DoEvents();
            }
        }

        public static void recordFailedLogin(string username, string msg)
        {
            string dateStr = Global.myNwMainFrm.cmnCdMn.getDB_Date_time();
            string[] mach_details = Global.myNwMainFrm.cmnCdMn.getMachDetails();
            string sqlStr = @"INSERT INTO sec.sec_track_user_logins(user_id, login_time, logout_time, host_mach_details, 
