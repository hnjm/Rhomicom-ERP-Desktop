...
to_char(to_timestamp(valid_end_date,'YYYY-MM-DD'),'DD-Mon-YYYY'), prsn_loc_id, org.get_site_code_desc(location_id) site_desc " +
...
to_timestamp(z.valid_end_date,'YYYY-MM-DD HH24:MI:SS'))))";
                ...
                else if (fltrTyp == "Site/Location")
                {
                    extra2 = @" and (EXISTS(SELECT REPLACE(w.location_code_name || '.' || w.site_desc, '.' || w.location_code_name,'') FROM pasn.prsn_locations z, org.org_sites_locations w 
WHERE (z.person_id = a.person_id and w.location_id = z.location_id and REPLACE(w.location_code_name || '.' || w.site_desc, '.' || w.location_code_name,'')='" + prsnTyp + @"'  
and now() between to_timestamp(z.valid_start_date,'YYYY-MM-DD HH24:MI:SS') and 
to_timestamp(z.valid_end_date,'YYYY-MM-DD HH24:MI:SS'))))";
                }
            }
